...
class SoftDotAttention_loss(nn.Module):
    '''Soft Dot Attention. 
    ...
    def __init__(self, query_dim, ctx_dim):
        '''Initialize layer.'''
        super(SoftDotAttention_loss, self).__init__()
        self.linear_in = nn.Linear(query_dim, ctx_dim, bias=False)
        # self.sm = nn.Softmax()
        self.sm = nn.Sigmoid()
        # self.linear_out = nn.Linear(query_dim + ctx_dim, query_dim, bias=False)
    ...
    def forward(self, h, context, loss, mask=None, end_mask=None,
                output_prob=True):
        '''Propagate h through the network.

        h: batch x seq_len x dim
        context: batch x seq_len x dim
        loss: batch x seq_len
        mask: batch x seq_len indices to be masked
        '''
        bs = h.size(0)
        dim_h = h.size(2)
        ...
        dim_ctx = context.size(2)
        context = context.view(-1, 1, dim_ctx) # batch*seq_len x 1 x dim
        

        target = self.linear_in(h).unsqueeze(2)  # batch*seq_len x dim x 1
        # print(context.shape, target.shape)
        # Get attention
        attn = torch.bmm(context, target)  # batch*seq_len x 1 x 1
        attn = attn.view(bs,-1)
        logit = attn

        # if mask is not None:
        #     # -Inf masking prior to the softmax
        #     mask = mask | (end_mask == 1)
        #     # attn1 = attn * (1 - mask.float())
        #     attn.masked_fill_(mask, -float('inf'))
        # print('before',attn)
        attn = self.sm(attn)
        # print(attn)
        ...
        if mask is not None:
            ...
            # attn1 = attn * (1 - mask.float())
            # attn.masked_fill_(mask, float('inf'))
            attn = attn * (1 - mask.float()) + mask.float() * 2
            min_v, _ = torch.min(attn, 1, True)
            # attn.masked_fill_(mask, -float('inf'))
            
            attn = attn * (1 - mask.float()) - mask.float() * 2
            range_v, _ = torch.max(attn, 1, True)
            range_v = range_v - min_v + 1e-5

            attn = 0.5 * (attn - min_v) / range_v
            # attn.masked_fill_(mask, 0)
            attn = ( attn + 0.5 ) * (1 - mask.float())
        

        # print('before error')
        # attn = self.sm(attn)    # There will be a bug here, but it's actually a problem in torch source code.
        # print('after error')
        attn = attn + end_mask.float()
        ...
        # print('ok',attn)
        # print('mask',mask)
        # print(range_v.squeeze())
        # print(min_v.squeeze())

        weighted_loss = torch.bmm(attn3, loss).squeeze(1)  # batch x dim

        if not output_prob:
            attn = logit
        
        return weighted_loss, attn

class SoftDotAttention_loss_sm(nn.Module):
    '''Soft Dot Attention. 
    ...
    def __init__(self, query_dim, ctx_dim):
        '''Initialize layer.'''
        super(SoftDotAttention_loss_sm, self).__init__()
        self.linear_in = nn.Linear(query_dim, ctx_dim, bias=False)
        self.sm = nn.Softmax()
        # self.sm = nn.Sigmoid()
        # self.linear_out = nn.Linear(query_dim + ctx_dim, query_dim, bias=False)
    ...
    def forward(self, h, context, loss, mask=None, end_mask=None,
                output_prob=True, base = 0.5, multi_length = True):
        '''Propagate h through the network.
        ...
        return weighted_loss, attn, loss.squeeze()
...
class Scoring(nn.Module):
    ...
    def __init__(self, vis_dim, ctx_dim):
        ...
        self.linear_vis = nn.Linear(vis_dim, ctx_dim, bias=False)
        self.linear_ctx = nn.Linear(ctx_dim, ctx_dim, bias=False)
        self.sm1 = nn.Softmax(dim=1)
        self.sm2 = nn.Softmax(dim=2)
        self.vis_dim = vis_dim
        self.ctx_dim = ctx_dim

    def forward(self, vis_f, ctx_f):
        '''Propagate h through the network.
        ...
        Hx = self.linear_ctx(ctx_f) # batch x dim
        Hv = self.linear_vis(vis_f) # batch x dim

        score = (Hx*Hv).sum(1) # batch
        score = score / torch.sqrt((Hx**2).sum(1) * (Hv**2).sum(1)) # batch
        return score

class AlignScoring(nn.Module):

    def __init__(self, vis_dim, ctx_dim):
        '''Initialize layer.'''
        super(AlignScoring, self).__init__()
        self.linear_vis = nn.Linear(vis_dim, ctx_dim, bias=False)
        self.linear_ctx = nn.Linear(ctx_dim, ctx_dim, bias=False)
    ...
    def forward(self, vis_f, ctx_f, vis_mask, ctx_mask):
        '''Propagate h through the network.

        vis_f: batch x n x vis_dim
        ctx_f: batch x m x dim
        vis_mask: batch x n
        ctx_mask: batch x m
        '''
        ...
        ctx_f = ctx_f.reshape(-1, self.ctx_dim)
        vis_f = vis_f.reshape(-1, self.vis_dim)

        Hx = self.lrelu(self.linear_ctx(ctx_f)) # batch * m x dim
        Hv = self.lrelu(self.linear_vis(vis_f)) # batch * n x dim

        
        Hx = Hx.reshape(-1, m, self.ctx_dim)
        Hv = Hv.reshape(-1, n, self.ctx_dim)

        ctx_mask_f = ctx_mask.repeat(self.ctx_dim,1,1).permute(1,2,0) # batch x m x dim
        ctx_mask_m = ctx_mask.repeat(n,1,1).permute(1,0,2) # batch x n x m
        # vis_mask = vis_mask.repeat(self.ctx_dim,1,1).permute(1,2,0) # batch x n x dim
        Hx.masked_fill_(ctx_mask_f, 0) 
        # Hv.masked_fill_(vis_mask, -float('inf'))
        # print('ctx_mask_f',ctx_mask_f)
        # print('ctx_mask_m',ctx_mask_m)
        # print('Hx',Hx)
        # print('Hv',Hv)

        Hx_mod = (Hx * Hx).sum(2,keepdim=True).sqrt() # batch * m * 1
        Hv_mod = (Hv * Hv).sum(2,keepdim=True).sqrt() # batch * n * 1

        Hx_mod_T = Hx_mod.permute(0,2,1)
        B = torch.bmm(Hv_mod, Hx_mod_T) + 1e-9 # batch * n * m

        Hx_T = Hx.permute(0,2,1) # batch x dim x m
        A_ = torch.bmm(Hv, Hx_T) # batch x n x m
        
        A = A_ / B * 5 # normalized

        # print('A',A)

        # A.masked_fill_(ctx_mask_m,-float('inf'))
        # print('A',A)
        sm_A = torch.sigmoid(A)
        # A_sm = self.sm2(A.masked_fill(ctx_mask_m,-float('inf')))
        A_sm = self.sm2(sm_A.masked_fill(ctx_mask_m,-float('inf')))
        
        # print('A_sm',A_sm)
        # print('A sigmoid',sm_A)
        C = (A_sm * sm_A).sum(2) # batch x n
        # print('C',C)
        # softmin
        score = (self.sm1((-1.0*C).masked_fill(vis_mask,-float('inf'))) * C).sum(1,keepdim=True) # batch x 1
        C = C * (1.0-vis_mask.float())
        # score = F.sigmoid(self.affine(score)) # batch x 1
        # print('score',score)
        return A, C, score

class AlignScoring_combine(nn.Module):

    def __init__(self, vis_dim, ctx_dim):
        '''Initialize layer.'''
        super(AlignScoring_combine, self).__init__()
        self.linear_vis = nn.Linear(vis_dim, ctx_dim, bias=False)
        self.linear_ctx = nn.Linear(ctx_dim, ctx_dim, bias=False)
        # self.affine = nn.Linear(1,1,bias=True)
        self.lrelu = nn.LeakyReLU(0.1)
    ...
    def forward(self, vis_f, ctx_f, mask):
        '''Propagate h through the network.

        vis_f: batch x n x vis_dim
        ctx_f: batch x n x dim
        mask: batch x n
        '''
        ...
        Hx = self.lrelu(self.linear_ctx(ctx_f)) # batch * n x dim
        Hv = self.lrelu(self.linear_vis(vis_f)) # batch * n x dim

        
        Hx = Hx.reshape(-1, n, self.ctx_dim)
        Hv = Hv.reshape(-1, n, self.ctx_dim)


        Hx_mod = (Hx * Hx).sum(2).sqrt() # batch x n
        Hv_mod = (Hv * Hv).sum(2).sqrt() # batch x n

        B = Hx_mod * Hv_mod + 1e-9 # batch * n


        A_ = (Hx * Hv).sum(2)
        
        A = A_ / B * 5 # normalized batch x n


        C = torch.sigmoid(A)

        # softmin
        score = (self.sm1((-1.0*C).masked_fill(mask,-float('inf'))) * C).sum(1,keepdim=True) # batch x 1
        C = C * (1.0-mask.float())
        # score = F.sigmoid(self.affine(score)) # batch x 1
        # print('score',score)
        return C, score
    
    def getscore(self, vis_f, ctx_f):
        '''Propagate h through the network.

        vis_f: batch x vis_dim
        ctx_f: batch x dim

        '''

        Hx = self.lrelu(self.linear_ctx(ctx_f)) # batch x dim
        Hv = self.lrelu(self.linear_vis(vis_f)) # batch x dim

        Hx_mod = (Hx * Hx).sum(1).sqrt() # batch
        Hv_mod = (Hv * Hv).sum(1).sqrt() # batch

        B = Hx_mod * Hv_mod + 1e-9 # batch

        A_ = (Hx * Hv).sum(1)
        
        A = A_ / B * 5 # normalized batch

        C = torch.sigmoid(A) # batch

        return A, C

class AlignScoring_sm(nn.Module):

    def __init__(self, vis_dim, ctx_dim):
        '''Initialize layer.'''
        super(AlignScoring_sm, self).__init__()
        self.linear_vis = nn.Linear(vis_dim, ctx_dim, bias=False)
        self.linear_ctx = nn.Linear(ctx_dim, ctx_dim, bias=False)
        # self.affine = nn.Linear(1,1,bias=True)
        self.lrelu = nn.LeakyReLU(0.1)
    ...
    def forward(self, vis_f, ctx_f, vis_mask, ctx_mask):
        ...
        # print('C',C)
        # softmin
        score = (self.sm1(C.masked_fill(vis_mask,-float('inf'))) * C).sum(1,keepdim=True) # batch x 1
        C = C * (1.0-vis_mask.float())
        # score = F.sigmoid(self.affine(score)) # batch x 1
        # print('score',score)
        return A, C, score

class AlignScoring_combine_sm(nn.Module):

    def __init__(self, vis_dim, ctx_dim):
        '''Initialize layer.'''
        super(AlignScoring_combine_sm, self).__init__()
        self.linear_vis = nn.Linear(vis_dim, ctx_dim, bias=False)
        self.linear_ctx = nn.Linear(ctx_dim, ctx_dim, bias=False)
        # self.affine = nn.Linear(1,1,bias=True)
        self.lrelu = nn.LeakyReLU(0.1)
    ...
    def forward(self, vis_f, ctx_f, mask):
        ...
        # softmin
        score = (self.sm1(C.masked_fill(mask,-float('inf'))) * C).sum(1,keepdim=True) # batch x 1
        C = C * (1.0-mask.float())
        # score = F.sigmoid(self.affine(score)) # batch x 1
        # print('score',score)
        return C, score
...
class GraphAttention(nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        self.linear_in = nn.Linear(dim_in, dim_in, bias=False)
        self.linear_out = nn.Linear(dim_in, dim_out,bias=False)
        self.sm1 = nn.Softmax(dim=2)
        

    def forward(self, feature, pool=False):
        '''
            feature: batch x num x dim

            return: 
                feature batch x num x dim
                attn num x num
                pool feature batch x dim
        '''

        attn = torch.bmm(self.linear_in(feature),feature.transpose(2,1)) # batch x num x num
        attn = self.sm1(attn) # batch x num x num
        out = self.linear_out(feature)
        # print(attn.shape, out.shape)
        feature = torch.bmm(attn, out).relu() # batch x num x dim_out

        if pool:
            feature = feature.max(1).values
...
#         '''
#             input: batch x num x dim
#         '''
#         feature = self.linear(input_feature)
