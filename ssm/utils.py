...
# from Mythread import MyThread, MyProcess
...
class Tokenizer(object):
    ...
    def split_sentence(sentence):
        ...
        toks = []
        for word in list([s.strip().lower() for s in Tokenizer.SENTENCE_SPLIT_REGEX.split(sentence.strip()) if len(s.strip()) > 0]):
    ...
    def encode_sentence(self, sentence, max_length=None):
        ...
        for word in self.split_sentence(sentence):
            encoding.append(self.word_to_index_f(word))   # Default Dict
